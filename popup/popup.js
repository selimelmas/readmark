/**
 * Readmark - High-Performance Popup Controller
 */
document.addEventListener('DOMContentLoaded', async () => {
  let items = [], currentFilter = 'all', searchQuery = '', settings = {}, sortMode = 'newest', dragSrcId = null, singleClickTimer = null, searchDebounce = null, toastTimer = null, confirmCallback = null, cachedTags = null;
  ...
  const getAllTags = () => {
    if (!cachedTags) cachedTags = Array.from(new Set(items.flatMap(i => i.tags || []))).sort((a, b) => a.localeCompare(b));
    return cachedTags;
  };
  const invalidateTags = () => { cachedTags = null; };
  ...
  window.addEventListener('pagehide', () => {
    clearTimeout(singleClickTimer); clearTimeout(searchDebounce); clearTimeout(toastTimer);
  });

  const updateBtn = (li, sel, base, act, on, offI, onI, offL, onL) => {
    const btn = li.querySelector(sel); if (!btn) return;
    const active = on(), lbl = active ? onL : offL;
    btn.className = active ? `${base} ${act}` : base; btn.innerHTML = active ? onI : offI;
    btn.title = lbl; btn.setAttribute('aria-label', lbl);
  };

  function updateSearchTagsBar() {
    const bar = $('searchTagsBar'), dl = $('allTagsDatalist'), allTags = getAllTags();
    if (dl) dl.innerHTML = allTags.map(tg => `<option value="#${esc(tg)}"></option>`).join('');
    if (!bar) return;
    bar.classList.toggle('hidden', !allTags.length);
    const cleanQ = searchQuery.replace(/^#/, '').toLowerCase();
    bar.innerHTML = allTags.map(tg => `<button class="search-tag-chip ${cleanQ === tg.toLowerCase() ? 'active' : ''}" data-tag="${esc(tg)}">#${esc(tg)}</button>`).join('');
  }

  function updateBadgesAndStats() {
    const unread = items.filter(i => !i.isRead).length, starred = items.filter(i => i.isPinned).length, read = items.length - unread;
  ...
  function editTitle(it, li) {
    ...
    const cur = it.title || '', input = Object.assign(document.createElement('input'), { type: 'text', className: 'item-title-input', value: cur });
    titleEl.replaceWith(input); input.focus(); input.select();

    let committed = false;
    const commit = async val => {
      if (committed) return; committed = true;
      const next = (val || '').trim() || cur, div = Object.assign(document.createElement('div'), { className: 'item-title', textContent: next });
    ...
    input.addEventListener('keydown', e => {
      ...
      if (e.key === 'Escape') { committed = true; input.replaceWith(Object.assign(document.createElement('div'), { className: 'item-title', textContent: cur })); }
    });
  }

  function addTagInput(it, li, container) {
    const addBtn = container.querySelector('.btn-add-tag'); if (!addBtn) return;
    const input = Object.assign(document.createElement('input'), { type: 'text', className: 'tag-input-inline', placeholder: t('tagPlaceholder', 'etiket...') });
    input.setAttribute('list', 'allTagsDatalist'); addBtn.replaceWith(input); input.focus();
    ...
    const commitTag = async val => {
      ...
      if (raw) {
        ...
        if (newTags.length !== curTags.length) { items = await Storage.updateItemTags(it.id, newTags); it.tags = newTags; showToast(t('tagSaved', 'Etiket eklendi')); }
      }
      renderList();
    };
  ...
  function renderList() {
    ...
    const filtered = items.filter(i => {
      ...
      return !searchQuery || (i.title || '').toLowerCase().includes(searchQuery) || (i.url || '').toLowerCase().includes(searchQuery) || (i.tags || []).some(tg => tg.toLowerCase().includes(cleanQ));
    });
    ...
    updateBadgesAndStats(); updateSearchTagsBar();
    ...
    const u = TIME_UNITS[I18n?.currentLang] || TIME_UNITS.en, now = Date.now(), frag = document.createDocumentFragment();
    ...
    filtered.forEach(it => {
      ...
      li.innerHTML = `
        ...
        <div class="item-content" title="${esc(it.title || it.url)}">
          ...
          <div class="item-tags">
            ${(it.tags || []).map(tg => `<span class="item-tag" data-tag="${esc(tg)}">#${esc(tg)}<span class="tag-del" data-action="del-tag" data-tag="${esc(tg)}" title="${t('removeTag','Kaldır')}">×</span></span>`).join('')}
  ...
  list?.addEventListener('click', async e => {
    ...
    if (!li) return;
    const it = items.find(i => i.id === li.dataset.id); if (!it) return;

    const tagDelBtn = e.target.closest('[data-action="del-tag"]');
    if (tagDelBtn) {
      e.stopPropagation();
      const newTags = (it.tags || []).filter(x => x !== tagDelBtn.dataset.tag);
      items = await Storage.updateItemTags(it.id, newTags); it.tags = newTags; renderList();
      return;
    }

    if (e.target.closest('[data-action="add-tag"]')) {
      e.stopPropagation(); addTagInput(it, li, e.target.closest('.item-tags')); return;
    }

    const tagSpan = e.target.closest('.item-tag');
    if (tagSpan) {
      e.stopPropagation(); toggleSearch(true);
      const q = '#' + tagSpan.dataset.tag;
      if ($('searchInput')) $('searchInput').value = q;
      searchQuery = q.toLowerCase(); $('btnClearSearch')?.classList.remove('hidden'); renderList();
      return;
    }
    ...
    if (actBtn) {
      ...
      if (act === 'read') { items = await Storage.toggleRead(it.id); updateCardDOM(li, items.find(i => i.id === it.id)); }
        ...
        const upd = items.find(i => i.id === it.id); updateCardDOM(li, upd);
    ...
    if (e.target.closest('.item-content') && e.target.tagName !== 'INPUT') {
      clearTimeout(singleClickTimer); if (e.detail > 1) return;
  ...
  list?.addEventListener('dblclick', e => {
    ...
    if (titleEl && li) {
      ...
      const it = items.find(i => i.id === li.dataset.id); if (it) editTitle(it, li);
    }
  });

  // Delegated Drag & Drop
  list?.addEventListener('dragstart', e => { const li = e.target.closest('.item-card'); if (li) { dragSrcId = li.dataset.id; li.classList.add('dragging'); e.dataTransfer.effectAllowed = 'move'; e.dataTransfer.setData('text/plain', dragSrcId); } });
  ...
  $('searchTagsBar')?.addEventListener('click', e => {
    const chip = e.target.closest('.search-tag-chip'); if (!chip) return;
    const nextQ = searchQuery.replace(/^#/, '').toLowerCase() === chip.dataset.tag.toLowerCase() ? '' : '#' + chip.dataset.tag;
    if ($('searchInput')) $('searchInput').value = nextQ;
    searchQuery = nextQ.toLowerCase(); $('btnClearSearch')?.classList.toggle('hidden', !searchQuery); renderList();
  });
